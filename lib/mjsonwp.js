...
function checkParams (paramSets, jsonObj) {
  ...
  if (!_.isArray(_.first(paramSets))) {
    paramSets = paramSets ? [paramSets] : [];
  }
  for (let params of paramSets) {
    if (_.difference(receivedParams, params).length === 0 &&
...
function makeArgs (reqParams, jsonObj, payloadParams) {
  ...
  return _.flatten(payloadParams).map(p => jsonObj[p])
